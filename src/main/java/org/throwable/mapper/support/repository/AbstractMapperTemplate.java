...
public abstract class AbstractMapperTemplate {
	...
	public boolean isNotEmpty() {
		return mapperTemplateAssistor.getConfig().isNotEmpty();
	}
	...
	protected String getSeqNextVal(EntityColumn column) {
		return MessageFormat.format(mapperTemplateAssistor.getConfig().getSeqFormat(), column.getSequenceName(), column.getColumn(), column.getProperty(), column.getTable().getName());
	}
	...
	protected void useGeneratedKeys(MappedStatement ms, EntityColumn column) {
		String keyId = ms.getId() + SELECT_KEY_SUFFIX;
